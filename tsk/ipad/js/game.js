...
var leveltext=[
			'1 of 6\nEnter into the castle and find\nthe first key!',
			'2 of 6',
			'3 of 6',
			'4 of 6',
			'5 of 6',
...
function start()
{
	...
	if(isMobile == 0)
	{
		...
		f('gui_soundon').addEventListener('mousedown', soundOff, false);
		f('gui_soundoff').addEventListener('mousedown', soundOn, false);
	...
	if(isMobile == 1)
	{
		...
		f('gui_soundon').addEventListener('touchstart', soundOff, false);
		f('gui_soundoff').addEventListener('touchstart', soundOn, false);
...
const STATECREDITS=5;
	*/
	...
	if(state == STATETITLE)
	{
		...
		ctx.drawImage(imgTitle, 0, 0, imgTitle.width, 85,      (getWidth()-imgTitle.width*2)/2, (getHeight()-85)*.3, 2*imgTitle.width, 2*85);
	}
	...
	{
		drawText('\n\n\nProgrammed by Istvan Szalontai\n\nAll rights reserved (C)2013');
		drawTitle(9);
	}
...
function drawTitle(n)
{
	...
	ctx.drawImage(imgTitle, 0, titleoffset[n], imgTitle.width, 85,      (getWidth()-imgTitle.width)/2, (getHeight()-90)*.3, imgTitle.width, 90);
}
...
function drawText(t)
{
	...
	for (i=0; i<rows; i++)
	{
		...
		texty = (getHeight()-HEIGHT*rows)/2+i*HEIGHT;
...
function keyPress(e)
{
	if(e.which==65 || e.which==37)speeds=-maxspeed*0.8;//d
	if(e.which==68 || e.which==39)speeds=maxspeed*0.8;//a
	if(e.which==87 || e.which==38)speedf=maxspeed;//w
	if(e.which==83 || e.which==40)speedf=-maxspeed;//s
	/*
...
function generateRooms(amount)
{
	...
	rooms[4]=0;
	rooms[5]=0;
	rooms[6]=(level-1)%3;//day 1-d 2-m 3-n 4-d 5-m 6-n
	rooms[7]=0;
	rooms[8]=0;
	rooms[9]=0;
	rooms[10]=0;
