...
document.addEventListener("keydown", function (event) {
  ...
  if (event.code == KEY_ESC && state == INGAME) {
    let left = JustBoxes.getActorByName(LEFT_PADDLE);
    left.score = 0;
    let right = JustBoxes.getActorByName(RIGHT_PADDLE);
    right.score = 0;
  ...
  if (event.code == KEY_P && state == INGAME) {
    paused = !paused;
  }

  if (event.code == KEY_ENTER && state == TITLE) {
...
document.addEventListener("keyup", function (event) {
  ...
  paddle = JustBoxes.getActorByName(LEFT_PADDLE);
  if (event.code == KEY_W || event.code == KEY_S) {
    paddle.yforce = 0;
  }
  if (event.code == KEY_A || event.code == KEY_D) {
    paddle.xforce = 0;
  }
});
...
function init() {
  ...
  JustBoxes.addActor(RIGHT_PADDLE, WIDTH - 6 * PIXEL, 5 * PIXEL, PIXEL, 5 * PIXEL);
