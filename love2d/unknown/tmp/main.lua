...
function drawWalls()
    ...
    for i=1,#walls do
        if walls[i] == 1 then
            love.graphics.setColor(192,0,192)
            love.graphics.rectangle("fill", ((i-1)%wallsWidth)*grid, math.floor((i-1)/wallsWidth)*grid,grid,grid)
        elseif walls[i] == 2 then
            love.graphics.setColor(0,0,0)
            love.graphics.rectangle("fill", ((i-1)%wallsWidth)*grid+grid/8*3, math.floor((i-1)/wallsWidth)*grid+grid/8*3,grid/4,grid/4)
        end
    end
end
