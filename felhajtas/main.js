...
release notes:
1.2
- Mozilla 3.6-os hiba jav�t�sa
- lek�t�z�tt �p�lettel is �tk�z�s kezel�se
- m�g�tt�nk l�v� dolgok ki nem rajzol�sa
- ront�s �s gy�zelem ut�n �jraindul a j�t�k

1.1
- gyalogl�s k�zben fejmozg�s
- els� �s m�sodik k�t�l felv�tele feliratai
...
var w, h;
var mag, fog, smallsize, behind, margin, tall, head;
	...
	do {
		if (border > world[i + 2])//y
		{
			...
			if (world[i] == 5 || world[i] == 0 ||
				world[i] == 2 || world[i] == 3 ||
				world[i] == 4 || world[i] == 8 ||
				world[i] == 7 || world[i] == 11) world[i + 3] += gravity;//lin�ris!!!
		}
		if (border > my_pos_y / mag && status != LIFT) status = FALL;//felsz�llt�l
...
function update() {
	...
	if (counter == 81) sendMessage('N�zz a h�tad m�g�! Hogy rep�lnek a f�k!', 50, 'K�zeledik a VIL�GV�GE! Menek�lj a v�ros fel�!', 100);
	if (my_pos_y > pplace * mag && my_pos_y < pplace * (mag + 1) && status == CAR) sendMessage('A k�zelben tal�lsz parkol� t�bl�t.', 50, 'Ott lerakhatod az aut�t.', 50);
	if (status == FALL) sendMessage('Felzuhant�l! Meg fogsz halni...', 200, 'Felzuhansz az �rbe, ahol majd megfulladsz :(', 1000);
	if (status == LIFT) {
...
function playerCollition() {
	...
	if (found == 1) {
		...
		if (world[i] == 2 && status == WALK) {
			sendMessage('Besz�llt�l a sikl�ba!', 50, 'Vigy�zz r� ne t�rd �ssze!', 50);
			world[i] = invisible;
			counter3 = 30;
			status = FLY;
			vehicle = i;
		...
		if (world[i] == 6 && status == WALK) {
			sendMessage('Ezzel te m�r nem rep�lsz ;-)', 50, 'Szerezz m�sik sikl�t!', 50);
		}
		//aut�
		if (world[i] == 3 && status == WALK) {
			if (hand_key == 1) {
				sendMessage('Besz�llt�l az aut�ba!', 50, '', 0);
				world[i] = invisible;
				status = CAR;
				vehicle = i;
			} else {
		...
		if (world[i] == 4 && status == CAR) {
			...
			status = WALK;
			sendMessage('Kisz�llt�l az aut�b�l!', 50, '', 0);
		}
		//h�zzal
		if (world[i] == 0 && hand_key == 0) {
			sendMessage('Szerezt�l slusszkulcsot!', 50, '', 0);
			hand_key = 1;
		}
		//V�rosh�z�val
...
function createWorld() {
	...
	do {
		for (var j = 0; j < prog[i + 1]; j++) {
			world[k] = prog[i + 0];
			if (world[k] == 8) must_saved++;//megsz�moljuk a toronyh�zakat
			if (world[k] == 4) pplace = prog[i + 3];//honnant�l lehet parkolni
...
function start() {
	...
	day_speed = .0003;//.0003 lass� volt
	day_begin = .5;//-1 este --> 1 nappal szinusz szerint v�ltozik
	fov = w / 2;
	invisible = 200;
